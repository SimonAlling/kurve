...
// Returns the number of items in an array that satisfy the given checker function,
// i.e. the number of x's in array for which checker(x) === true:
function numberOfMatching(array, checker) {
    var n = 0;
    for (var i = 0, len = array.length; i < len; i++) {
        if (checker.call(this, array[i]) === true) {
            n++;
        }
    }
    return n;
...
function demand(object, checker) {
    if (checker.call(this, object) === true) {
        return object;
    } else {
        return undefined;
    }
